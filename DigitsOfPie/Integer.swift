...
//        }
    ...
    func rShift (_ n: Int) -> Integer {
        ...
        if negative {
            ...
            if newSize <= 0 {
                ...
                let hiMask = Integer.mask ^ loMask
                var z = Integer(size: newSize)
                var j = wordShift
                for i in 0..<newSize {
    ...
    fileprivate static func signed (_ z: inout Integer) {
        ...
        let signMask = 1 << (shift-1)   // sign bit mask
        if size == 0 { return }
        z.negative = Int(z.digit[size-1]) & signMask != 0
...
extension Integer : BinaryInteger {
    ...
    public static func <<= <RHS>(lhs: inout Integer, rhs: RHS) where RHS : BinaryInteger { lhs = lhs << Int(rhs) }
    public static func >>= <RHS>(lhs: inout Integer, rhs: RHS) where RHS : BinaryInteger { lhs = lhs >> Int(rhs) }
